...
class Mysql {
	...
	public function repair( $table ) {
		return Db::execute( "REPAIR TABLE `" . Config::get( 'database.prefix' ) . $table . "`" );
	}
	...
	public function optimize( $table ) {
		return Db::execute( "OPTIMIZE TABLE `" . Config::get( 'database.prefix' ) . $table . "`" );
	}
	...
	public function tableExists( $tableName ) {
		...
		foreach ( $tables as $k => $table ) {
			...
			if ( strtolower( $table[ $key ] ) == strtolower( Config::get( 'database.prefix' ) . $tableName ) ) {
				return true;
			}
		}
